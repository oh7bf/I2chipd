...
void ReadTmp102_3()
{
  ...
  const char tdatafile[200]="/var/lib/i2chipd/tmp102_3_temperature";
  ...
  value = I2cReadBytes(TMP102_ADDRESS3, 2);
  if( value >= 0 )
  {
    temp = (short)(value);
    temp /= 16;
    ...
    sprintf(message, "TMP102_3 T=%+6.2f C", temperature);
    syslog(LOG_INFO|LOG_DAEMON, "%s", message);

    WriteFile( tdatafile, temp);

    data[0] = (float)temperature;
    if( dbsqlite == 1 ) InsertSQLite( dbfile, query, "T3", 1, data);
    if( dbmysql == 1 ) InsertMySQL(dbhost, dbuser, dbpswd, database, dbport, mquery, "T3", 1, data);
  }
  else
  {
    syslog(LOG_ERR|LOG_DAEMON, "TMP102_3 temperature reading failed");
  }
}
